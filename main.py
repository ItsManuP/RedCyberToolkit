...
def build_orchestrator(target: str, authorized: bool, opts: dict) -> Orchestrator:
    orch = Orchestrator(target=target, authorized=authorized, options=opts)
    ...
    orch.enqueue(AttackPhase.AUTH_ATTACK, "auth", {"max_attempts": opts.get("max_attempts",100), "protocols":["ssh","ftp","http","mysql","postgresql"]})
...
def main():
    print(BANNER)
    parser = argparse.ArgumentParser(description="RedTeam Toolkit — Multi-Agent Pentest Framework")
    parser.add_argument("--target", required=True, help="IP address o URL del target")
    parser.add_argument("--phases", default="all", help="Fasi: all | discovery,cve,auth,exploit,traffic,report")
    ...
    parser.add_argument("--yes", action="store_true", help="Salta prompt autorizzazione")
    args = parser.parse_args()

    authorized = True if args.yes else ask_authorization(args.target)
    if not authorized:
        print("\n[ABORT] Operazione annullata — autorizzazione non confermata.")
        sys.exit(1)
    ...
    opts = {"port_range": args.port_range, "use_nmap": not args.no_nmap,
            "safe_mode": args.safe_mode, "use_nvd_api": args.use_nvd_api,
            "max_attempts": args.max_attempts}

    orch = build_orchestrator(args.target, authorized=True, opts=opts)
